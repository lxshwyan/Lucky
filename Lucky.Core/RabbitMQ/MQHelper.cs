...
namespace Lucky.Core.RabbitMQ
{
    public class MQHelper: BusBuilder {
        ...
        #region 默认"topic"   ----建议使用
        /// <summary>
        /// 获取主题消息（系统默认生成交换机名称）----建议使用
        /// </summary>
        /// <typeparam name="T">主题内容类型</typeparam>
        /// <param name="subscriptionId">订阅者ID</param>
        /// <param name="callback">消息接收响应回调</param>
        ///  <param name="AutoDelete">是否自动删除（默认删除）</param>
        ///  <param name="topics">订阅主题集合</param>   
        public void TopicSubscribe(string subscriptionId, Action<RabbitMQMsg> callback, bool AutoDelete = true, params string[] topics)
        {
          
               var bus = CreateMessageBus();    
                bus.Subscribe(subscriptionId, callback, (config) =>
                {
                    config.WithAutoDelete(AutoDelete);
                    foreach (var item in topics) config.WithTopic(item);
                });
           
        }
        /// <summary>
        /// 发布主题消息（系统默认生成交换机名称） -----建议使用
        /// </summary>
        /// <typeparam name="T">主题内容类型</typeparam>
        ...
        /// <returns></returns>
        public bool TopicPublish(string topic, RabbitMQMsg message, ref string strErrormsg)
        {
            try
            {
