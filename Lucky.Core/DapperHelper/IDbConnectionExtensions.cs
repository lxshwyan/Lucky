...
*文件名： IDbConnectionExtensions
*创建人： Lxsh
*创建时间：2019/1/15 11:43:45
*描述
*=======================================================================
*修改标记
*修改时间：2019/1/15 11:43:45
*修改人：Lxsh
*描述：
************************************************************************/
...
namespace Lucky.Core.DapperHelper
{
    ...
    public static class IDbConnectionExtensions
    {
        public static List<DbTable> GetCurrentDatabaseTableList(this IDbConnection dbConnection, DatabaseType dbType)
        {
            ...
            tables.ForEach(item =>
            {
                ...
                item.Columns.ForEach(c =>
                {
                    ...
                    string csharpType = DbColumnTypeCollection.DbColumnDataTypes.FirstOrDefault(t=>
                          t.DatabaseType==dbType&&t.ColumnTypes.Split(',').Any(p =>
                              p.Trim().Equals(c.ColumnType, StringComparison.OrdinalIgnoreCase)))?.CSharpType;
                    if (string.IsNullOrEmpty(csharpType))
                    {
                        throw new SqlTypeException($"未从字典中找到\"{c.ColumnType}\"对应的C#数据类型，请更新DbColumnTypeCollection类型映射字典。");
                    }
                    c.CSharpType = csharpType;
                }); 
            });
            return tables;
        }
        ...
        private static List<DbTable> GetCurrentDatabaseAllTables(this IDbConnection dbConnection, DatabaseType dbType)
        {
            ...
            return dbConnection.Query<DbTable>(dbConnection.GetAllTablesSql(dbType)).ToList();
        ...
        private static List<DbTableColumn> GetColumnsByTableName(this IDbConnection dbConnection, DatabaseType dbType, string tableName)
        {
            if (dbConnection == null) throw new ArgumentNullException(nameof(dbConnection));
            if (dbConnection.State == ConnectionState.Closed) dbConnection.Open();
            return dbConnection.Query<DbTableColumn>(dbConnection.GetAllColumnsSql(dbType, tableName)).ToList();
        }
        /// <summary>
        /// 根据数据库类型获取数据库中数据库类型获取所有表对应sql语句
        /// </summary>
        /// <param name="dbConnection">数据库连接</param>
        /// <param name="dbType">数据库类型</param>
        /// <returns></returns>
        private static string GetAllTablesSql(this IDbConnection dbConnection, DatabaseType dbType)
        {
...
FROM      sys.syscolumns AS a LEFT OUTER JOIN
        ...
        /// <summary>
        ///   根据数据库类型获取数据库中表名及数据库类型获取所有列对应sql语句
        /// </summary>
        /// <param name="dbConnection">数据库连接</param>
        /// <param name="dbType">数据库连接</param>
        /// <param name="tableName">数据库表</param>
        /// <returns></returns>
        private static string GetAllColumnsSql(this IDbConnection dbConnection, DatabaseType dbType, string tableName)
        {
            var strGetTableColumns = string.Empty;
            switch (dbType)
            {
                case DatabaseType.SqlServer:
                    strGetTableColumns = $@"SELECT   a.name AS ColumnName, CONVERT(bit, (CASE WHEN COLUMNPROPERTY(a.id, a.name, 'IsIdentity') 
